...
#homocesdacityCheckfunction
homocesdasticityCheck<-function(nonlinear_model){
  ...
  print(bp_test_result[1])#debug
  print(bp_test_result[2])#debug
  print(bp_test_result[3])#debug
  print(bp_test_result[4])#debug
  #TODO: Check if any is sensible here.
  aggregation <- aggregate(Language,list(Language$vertices),mean)
  if(bp_test_result[4] < 0.05){#if lower than 0.05 indicating the presence of   heteroscedasticity,  take aggregated values
    #aggregation <- aggregate(. ~ vertices, data = Language, mean)
    aggregation <- aggregate(Language,list(Language$vertices),mean)
    
    print("TODO:THESE HAVE TO BE CHANGED IN ACCORDANCE TO THE REQUIRED MODEL.
          CURRENTLY THIS IS JUST MODEL 2!")
    linear_model <- lm(log(degree_2nd_moment)~log(vertices), aggregation)
    a_initial <- exp(coef(linear_model)[1])
    b_initial <- coef(linear_model)[2]
    
    nonlinear_model <- nls(degree_2nd_moment ~ a * vertices^b, data = aggregation, start = list(a = a_initial, b = b_initial), trace = TRUE)
...
while(i<length(languages)+1){
  ...
  RSSVector[1] <- languages[i]
  AICVector[1] <- languages[i]
  AICDeltaVector[1] <- languages[i]
  ParameterVector[1] <- languages[i]
  
  print(languages[i])
  string<-paste0(languages[i], "_dependency_tree_metrics.txt")
  Language = read.table(string, header = FALSE)
  ...
  a_initial <- exp(coef(linear_model)[1])
  #2 f(n) = (n/2)^b
  nonlinear_model <- nls(degree_2nd_moment~(vertices/2)^b,data=Language,
  start <- list(b = b_initial), trace = TRUE)
  #3 
  nonlinear_model <- homocesdasticityCheck(nonlinear_model)
  #4
  ...
  b_initial <- coef(linear_model)[2]
  #2
  nonlinear_model <- nls(degree_2nd_moment~a*vertices^b,data=Language,
  ...
  #Calculating Model 3 #Mandatory #TODO
  
  #1
  linear_model <- lm(log(degree_2nd_moment)~vertices, Language)
  a_initial <- exp(coef(linear_model)[1])
  c_initial <- coef(linear_model)[2]
  #2 f(n) = ae^{cn} #E IS EXPONENT, VERTICES IS N
  nonlinear_model <- nls(degree_2nd_moment~a*exp(c*vertices),data=Language, 
  ...
  AICVector[6] <- "-"
  AICDeltaVector[6] <- "-"
  ParameterVector[7] <- "-"#4a
  
  #Calculating Model 1+ #Mandatory
  #1
  linear_model <- lm(log(degree_2nd_moment)~log(vertices), Language)
  a_initial <- exp(coef(linear_model)[1])
  d_initial_1p = 0 #idk what to put here tbh tbh
  #2 f(n) = (n/2)^b
  nonlinear_model <- nls(degree_2nd_moment~(vertices/2)^b+d,data=Language,
  start <- list(b = b_initial, d=d_initial_1p), trace = TRUE)
  #3 
  nonlinear_model <- homocesdasticityCheck(nonlinear_model)
  #4
  ...
  #Calculating Model 2+ #Advanced Exercises #Careful Choice for initial parameters
  RSSVector[8] <- "-"
  AICVector[8] <- "-"
  AICDeltaVector[8] <- "-"
  ...
  #Calculating Model 3+ #Advanced Exercises #Careful choice for initial parameters
  RSSVector[9] <- "-"
  AICVector[9] <- "-"
  AICDeltaVector[9] <- "-"
  ...
  AICVector[10] <- "-"
  AICDeltaVector[10] <- "-"
  ParameterVector[16] <- "-"#3+a
  ParameterVector[17] <- "-"#3+d
  
  akaikeInformationCriterionTable[i,] <- AICVector
  columnparametertable[i,] <- ParameterVector
  residualStandardErrorTable[i,] <- RSSVector
  aicdifferenceTable[i,] <- AICDeltaVector
  i<-i+1
}
