...
homocesdasticity_check <- function(lang_data, nonlinear_model, mdl,
  ...
  print(sprintf("p-value: %f", bp_test_result[4]))
  print("bp test end")
...
main <- function(lang, filename, index) {
  ...
  #print(head(var_values))
  #if ((max(var_values$K2) / min(var_values$K2)) > 2) {
  #  lang_data <- mean_values
  ...
  AIC_best <- min(c(na.omit(as.numeric(AICVector))))
...
plot_res <- function(lang, filename, i) {
  ...
  plot(lang_data$N, lang_data$K2,
         main=sprintf("Model: %i, %s", mdl-2, lang), log="xy")
    if (mdl == 2) { # f(n) = (1-1/n)*(5-6/n)
      lines((1 - 1 / lang_data$N) * (5 - 6 / lang_data$N), type="l", col="red")
